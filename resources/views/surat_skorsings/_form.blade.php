...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div class="md:col-span-2">
        ...
        <input type="text" name="nomor_surat" value="{{ old('nomor_surat', $s->nomor_surat ?? '421.5 / 042 / SMKN1-Kb/DINDIK / 2026') }}" class="w-full border rounded px-3 py-2 dark:bg-gray-700 dark:text-gray-100">
    </div>
    ...
    <div class="md:col-span-2">
        <h2 class="text-lg font-semibold mb-4 border-b pb-2 mt-4">Detail Skorsing</h2>
    </div>
    <div class="md:col-span-2">
        <label class="block font-medium mb-1">Pelanggaran</label>
        <textarea name="pelanggaran" rows="3" class="w-full border rounded px-3 py-2 dark:bg-gray-700 dark:text-gray-100">{{ old('pelanggaran', $s->pelanggaran ?? '') }}</textarea>
    </div>
    <div>
        <label class="block font-medium mb-1">Durasi Skorsing</label>
        <input type="text" name="durasi_skorsing" value="{{ old('durasi_skorsing', $s->durasi_skorsing ?? '2 Minggu (14 hari)') }}" placeholder="Contoh: 2 Minggu (14 hari)" class="w-full border rounded px-3 py-2 dark:bg-gray-700 dark:text-gray-100">
    </div>
    <div>
        <label class="block font-medium mb-1">Tanggal Mulai</label>
        <input type="date" name="tanggal_mulai" value="{{ old('tanggal_mulai', optional($s->tanggal_mulai ?? null)->format('Y-m-d')) }}" class="w-full border rounded px-3 py-2 dark:bg-gray-700 dark:text-gray-100">
    </div>
    <div>
        <label class="block font-medium mb-1">Tanggal Selesai</label>
        <input type="date" name="tanggal_selesai" value="{{ old('tanggal_selesai', optional($s->tanggal_selesai ?? null)->format('Y-m-d')) }}" class="w-full border rounded px-3 py-2 dark:bg-gray-700 dark:text-gray-100">
    </div>
    ...
    <div class="md:col-span-2">
        ...
        <select name="penandatangan_id" class="w-full border rounded px-3 py-2 dark:bg-gray-700 dark:text-gray-100">
            ...
            @foreach($asns as $asn)
                <option value="{{ $asn->id }}" {{ old('penandatangan_id', $s->penandatangan_id ?? $defaultPenandatanganId) == $asn->id ? 'selected' : '' }}>
